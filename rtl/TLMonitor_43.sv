  ...
  `ifndef SYNTHESIS
    ...
    always @(posedge clock) begin
      ...
      if (~reset & ~(a_set_wo_ready != d_clr_wo_ready | ~a_set_wo_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'A' and 'D' concurrent, despite minlatency 5 (connected at generators/boom/src/main/scala/common/tile.scala:138:21)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
      end
