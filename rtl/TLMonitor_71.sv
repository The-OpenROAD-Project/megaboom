...
module TLMonitor_71(
  ...
  `ifndef SYNTHESIS
    ...
    wire        _GEN_20 = _GEN_1 & (_GEN_12 | _GEN_13 & (_GEN_14 | _GEN_15 | _GEN_16 | _GEN_17 | _GEN_2 | _GEN_18 | _GEN_3 | _GEN_4));
    wire        _GEN_21 = io_in_a_valid & io_in_a_bits_opcode == 3'h0 & ~reset;
    wire        _GEN_22 = io_in_a_valid & io_in_a_bits_opcode == 3'h1 & ~reset;
    wire        _GEN_23 = _GEN_1 & io_in_a_bits_size < 4'h4 & (_GEN_14 | _GEN_11 | _GEN_15 | _GEN_16 | _GEN_17 | _GEN_2 | _GEN_18 | _GEN_3 | _GEN_4);
    ...
    always @(posedge clock) begin
      ...
      if (_GEN_10 & ~(_GEN_12 | _GEN_13 & (_GEN_14 | {io_in_a_bits_address[32:17], ~(io_in_a_bits_address[16])} == 17'h0 | _GEN_15 | _GEN_16 | _GEN_17 | _GEN_2 | _GEN_18 | _GEN_3 | _GEN_4))) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (_GEN_26 & ~(_GEN_1 & (_GEN_12 | _GEN_13 & _GEN_5))) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (_GEN_39 & a_first_1 & io_in_a_valid & ~io_in_d_bits_source & ~d_release_ack & ~reset & ~(~io_in_d_ready | io_in_a_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (~reset & ~(a_set_wo_ready != (_GEN_40 & ~io_in_d_bits_source) | ~a_set_wo_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'A' and 'D' concurrent, despite minlatency 9 (connected at generators/testchipip/src/main/scala/tsi/TSIHarness.scala:77:45)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
      end
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      inflight <= (inflight | a_set) & ~(_GEN_47 & ~io_in_d_bits_source);
      inflight_opcodes <= (inflight_opcodes | (a_set ? {io_in_a_bits_opcode, 1'h1} : 4'h0)) & ~(_GEN_47 ? _d_opcodes_clr_T_5[3:0] : 4'h0);
      inflight_sizes <= (inflight_sizes | (a_set ? {3'h0, a_set ? {io_in_a_bits_size, 1'h1} : 5'h0} : 8'h0)) & ~(_GEN_47 ? _d_sizes_clr_T_5[7:0] : 8'h0);
