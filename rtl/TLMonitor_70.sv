...
module TLMonitor_70(
  ...
  `ifndef SYNTHESIS
    ...
    wire        _GEN_23 = _GEN_3 & (_GEN_15 | _GEN_16 & (_GEN_17 | _GEN_18 | _GEN_19 | _GEN_20 | _GEN_4 | _GEN_21 | _GEN_5 | _GEN_6));
    wire        _GEN_24 = io_in_a_valid & io_in_a_bits_opcode == 3'h0 & ~reset;
    wire        _GEN_25 = io_in_a_valid & io_in_a_bits_opcode == 3'h1 & ~reset;
    wire        _GEN_26 = _GEN_3 & io_in_a_bits_size < 4'h4 & (_GEN_17 | _GEN_14 | _GEN_18 | _GEN_19 | _GEN_20 | _GEN_4 | _GEN_21 | _GEN_5 | _GEN_6);
    ...
    wire [7:0]  _GEN_41 = {4'h0, io_in_d_bits_source, 3'h0};
    ...
    wire        _GEN_44 = _GEN_43 & same_cycle_resp & ~reset;
    ...
    wire [7:0]  _a_size_lookup_T_1 = inflight_sizes >> _GEN_41;
    wire [7:0]  _c_size_lookup_T_1 = inflight_sizes_1 >> _GEN_41;
    always @(posedge clock) begin
      ...
      if (_GEN_13 & ~(_GEN_15 | _GEN_16 & (_GEN_17 | {io_in_a_bits_address[32:17], ~(io_in_a_bits_address[16])} == 17'h0 | _GEN_18 | _GEN_19 | _GEN_20 | _GEN_4 | _GEN_21 | _GEN_5 | _GEN_6))) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (_GEN_29 & ~(_GEN_3 & (_GEN_15 | _GEN_16 & _GEN_7))) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (_GEN_43 & ~reset & ~(inflight >> io_in_d_bits_source | same_cycle_resp)) begin
        if (`ASSERT_VERBOSE_COND_)
      ...
      if (~reset & ~(a_set_wo_ready != (_GEN_43 & ~io_in_d_bits_source) | ~a_set_wo_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'A' and 'D' concurrent, despite minlatency 7 (connected at generators/testchipip/src/main/scala/tsi/TSIHarness.scala:77:31)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
      end
      ...
      if (_GEN_47 & ~(inflight_1 >> io_in_d_bits_source)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'D' channel acknowledged for nothing inflight (connected at generators/testchipip/src/main/scala/tsi/TSIHarness.scala:77:31)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
  ...
  wire [30:0] _GEN_49 = {27'h0, io_in_d_bits_source, 3'h0};
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      inflight <= (inflight | _GEN_0 & ~io_in_a_bits_source) & ~(_GEN_50 & ~io_in_d_bits_source);
      inflight_opcodes <= (inflight_opcodes | (_GEN_0 ? _a_opcodes_set_T_1[3:0] : 4'h0)) & ~(_GEN_50 ? _d_opcodes_clr_T_5[3:0] : 4'h0);
      inflight_sizes <= (inflight_sizes | (_GEN_0 ? _a_sizes_set_T_1[7:0] : 8'h0)) & ~(_GEN_50 ? _d_sizes_clr_T_5[7:0] : 8'h0);
      if (_a_first_T_1 | _d_first_T_2)
        watchdog <= 32'h0;
      else
        watchdog <= watchdog + 32'h1;
      inflight_1 <= inflight_1 & ~(_GEN_51 & ~io_in_d_bits_source);
      inflight_sizes_1 <= inflight_sizes_1 & ~(_GEN_51 ? _d_sizes_clr_T_11[7:0] : 8'h0);
    end
