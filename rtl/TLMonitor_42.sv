...
module TLMonitor_42(
  ...
  `ifndef SYNTHESIS
    ...
    wire        _GEN_84 = io_in_c_bits_size < 4'h3;
    ...
    always @(posedge clock) begin
      ...
      if (~reset & ~(a_set_wo_ready != (_GEN_102 ? _d_clr_wo_ready_T[9:0] : 10'h0) | a_set_wo_ready == 10'h0)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'A' and 'D' concurrent, despite minlatency 5 (connected at generators/boom/src/main/scala/common/tile.scala:132:21)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
      end
      ...
      if ((|c_set_wo_ready) & ~reset & c_set_wo_ready == (_GEN_108 ? _d_clr_wo_ready_T_1[9:0] : 10'h0)) begin
        if (`ASSERT_VERBOSE_COND_)
          $error("Assertion failed: 'C' and 'D' concurrent, despite minlatency 5 (connected at generators/boom/src/main/scala/common/tile.scala:132:21)\n    at Monitor.scala:49 assert(cond, message)\n");
        if (`STOP_COND_)
          $fatal;
      end
