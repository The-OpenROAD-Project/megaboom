...
class CredHelperTests(unittest.TestCase):
    ...
    def test_test_mode(self):
        ...
        """
        result = subprocess.run([self._exe, "test"], stdout=subprocess.PIPE,
        ...
        try:
            ...
            self.assertTrue("storage.buckets.get" in json_data["permissions"],
                            "{} is missing read access".format(user_name))
            self.assertTrue("storage.objects.create" in json_data["permissions"],
    ...
    def test_get_mode(self):
        ...
        the user in the .user-bazelrc has the correct entitlements
        """
        result = subprocess.run([self._exe, "get"], stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE, text=True)
        self.assertEqual(result.returncode, 0,
        ...
        try:
            json_data = json.loads(result.stdout)
            self.assertIsInstance(json_data["headers"]["Authorization"], list,
    ...
    def test_usage_mode(self):
        ...
        """
        result = subprocess.run([self._exe], stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE, text=True)
